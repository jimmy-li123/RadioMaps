...
def main():
    args = parse_args(description="Evaluate Channel Knowledge Map (CKM) baseline")
    ...
    # Load data arrays
    UEloc, CSI, LoS, AoD = load_dataset(args.dataset, return_aod=True)
    ...
    CKM_LoS_SE, CKM_NLoS_SE = compute(CKM_SE, LoS_test)                                                  
    opt_LoS_SE, opt_NLoS_SE = compute(opt_SE, LoS_test)                                                  
                                                                                                            
    CKM2opt_LoS = (CKM_LoS_SE / opt_LoS_SE) * 100 if opt_LoS_SE > 0 else 0.0                             
    CKM2opt_NLoS = (CKM_NLoS_SE / opt_NLoS_SE) * 100 if opt_NLoS_SE > 0 else 0.0  

    # Print result
    print(f"=== CKM Baseline ({args.dataset}) ===")
